...
This file owns retained CLI and protocol behavior plus backend contracts.
Checked requirements describe the current baseline; unchecked requirements
describe the remaining CLI and printing work.

## Implementation Sequence

1. Protect the retained CLI, BLE, protocol, and raster baseline.
2. Define capability, document, application-service, and fake-adapter contracts.
3. Extract deterministic rendering and font resolution shared by preview and print.
4. Expose typed printer lifecycle snapshots and events.
5. Integrate validated print settings, jobs, cancellation, and safe retry.
6. Move the CLI onto the shared application services.
...
7. [ ] The CLI contains shared capability, validation, rendering, and print-job services.
...
6. [ ] The application contains one authoritative capability registry shared by the CLI, renderer, and printer services.
...
5. [ ] The application contains one deterministic renderer shared by CLI export and physical printing.
6. [ ] The CLI can export the exact monochrome raster sent to the selected printer.
7. [ ] The CLI export contains selected orientation, rotation, threshold or dithering, offsets, printable bounds, and pixel dimensions.
8. [ ] The renderer contains fixture-tested pixel parity between exported and printer output.
...
3. [ ] The application contains physical printing through the CLI.
...
5. [ ] The CLI print flow reports printer, label, dimensions, orientation, monochrome output, copies, density, offsets, and warnings before printing.
6. [ ] CLI print settings are read and validated when the command runs.
7. [ ] Print jobs contain queued, discovering, connecting, preparing, sending, printing, completed, cancelled, and failed states.
8. [ ] Print jobs contain console progress, cancellation, bounded timeout, safe retry, and sanitized diagnostics.
...
9. [ ] The application contains safe one-way migration from legacy `.niim` files without exposing pickle loading through normal CLI commands.

...
function hdiutil_detach_retry() {
	...
	until
		...
		(( unmounting_attempts++ ))
		cd "$SCRIPT_DIR" || exit 1  # Change directory to one level up
		hdiutil detach "$1"
		exit_code=$?
...
for i in {1..5}; do
    kill_xprotect
    unmount_all_disks
    echo "Attempt $i: Creating disk image..."
    ...
    if [[ $SANDBOX_SAFE -eq 0 ]]; then
        ...
        if hdiutil create ${HDIUTIL_VERBOSITY} -srcfolder "$SRC_FOLDER" -volname "${VOLUME_NAME}" \
            -fs "${FILESYSTEM}" -fsargs "${FILESYSTEM_ARGUMENTS}" -format UDRW ${CUSTOM_SIZE} "${DMG_TEMP_NAME}"; then
...
# Ensure we are not in the directory before detaching
